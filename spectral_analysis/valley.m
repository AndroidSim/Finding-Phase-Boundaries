function [B,I,indices] = valley(spectra,B0)
% find magnetic field value of nearest valley to B0 field value
...
for i =1:ns
    ...
    if sign(Gb) ~= sign(Ga)
        % find index that has a lower value 
    ...
    % find bottom of nearest valley following the gradient
    while ~bottom % (sign(Gb) == sign(Ga)) == (Gb < 0 & Ga > 0) | (Gb > 0 & Ga < 0)
        if all(sign([Gb Ga]) == 1)
            indb = inda;
            Gb = G(indb);
            inda = indb-1;
